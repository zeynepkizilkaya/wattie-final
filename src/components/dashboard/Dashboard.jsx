...
export default function Dashboard() {
  ...
  const activeHome = selectedHomeId
    ? homes.find((h) => String(h.id) === String(selectedHomeId))
    : null;
   ...
   *   quotaTry
   *
   * ?? kullanıyoruz ki undefined değerler
   * Dashboard'u çökertmesin.
   */

  const displayKwh = activeHome
    ? (activeHome.usedKwh ?? 0)
    : homes.reduce((sum, h) => sum + (h.usedKwh ?? 0), 0);

  const displayQuotaKwh = activeHome
    ? (activeHome.quotaKwh ?? activeHome.powerQuotaKwh ?? 0)
    : homes.reduce(
        (sum, h) => sum + (h.quotaKwh ?? h.powerQuotaKwh ?? 0),
        0
      );

  const displayTry = activeHome
    ? (activeHome.usedTry ?? 0)
    : homes.reduce((sum, h) => sum + (h.usedTry ?? 0), 0);

  const displayQuotaTry = activeHome
    ? (activeHome.quotaTry ?? activeHome.financialQuota ?? 0)
    : homes.reduce(
        (sum, h) => sum + (h.quotaTry ?? h.financialQuota ?? 0),
        0
      );
  ...
  const topDevices = activeHome?.appliances
    ? activeHome.appliances
        ...
        .map((a) => ({
          ...
          pct: Math.min(
            100,
            Math.round(
              (a.currentWatt / (a.safeWatt || 1)) * 100
            )
          ),
          color: a.isAnomalous
            ? "var(--danger)"
    ...
    : [
        ...
        {
          name: "Çamaşır Makinesi (Banyo)",
          kw: "0.78",
          pct: 18,
          color: "var(--current)",
        },
        {
          name: "Televizyon (Oturma O.)",
          kw: "0.21",
          pct: 12,
          color: "var(--text-2)",
        },
  ...
  return (
    <div className="dashboard-page-container">
      ...
      <HomeSelector
        homes={homes}
        onAddHomeClick={() => setIsAddHomeModalOpen(true)}
      />
      ...
      <div className="dashboard-top-widgets">
        ...
        <div className="widget-card glass-panel clock-widget">
          ...
          <span className="clock-date">
            {dateStr || "25 Temmuz 2026 Cumartesi"}
          </span>
        </div>
        ...
        <div className="widget-card glass-panel stat-widget">
          ...
          <div className="widget-content">
            <span className="widget-label">
              {activeHome
                ? `${activeHome.name} Anlık Tüketim`
                : "Anlık Toplam Tüketim"}
            </span>

            <strong className="widget-value mono">
              {displayKw} kW
            </strong>

            <span className="widget-trend success">
              ↓ %12 düne göre
            </span>
          </div>
        </div>
        ...
        <div className="widget-card glass-panel stat-widget">
          <div className="widget-icon-box volt">
            TRY
          </div>
          ...
          <div className="widget-content">
            ...
            <strong className="widget-value mono">
              ₺{displayTry.toLocaleString("tr-TR")}
            </strong>

            <span className="widget-sub">
              Bütçe: ₺
              {displayQuotaTry.toLocaleString("tr-TR")}
            </span>
          </div>
        </div>
      </div>

      {/* 3. Central 3D Street View Hero */}

      <div className="glass-panel house-preview-3d-panel">
        <div className="preview-3d-overlay">
          <span className="overlay-badge mono">
            {activeHome
              ? `${activeHome.name.toUpperCase()}: ${displayKw} kW`
              : `SOKAK TÜKETİMİ: ${displayKw} kW`}
          </span>
        </div>
        ...
        <div className="preview-3d-canvas-wrapper">
          <House3D
            ...
            onSelectHome={(home) => handleSelectHome(home)}
          />
        </div>
      </div>
